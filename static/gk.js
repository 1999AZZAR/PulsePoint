...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        resultsDiv.innerHTML = `
            ...
            <div class="bg-white p-6 rounded-lg shadow tags-card">
                <h2 class="text-xl font-semibold mb-2">
                    <i class="fa-solid fa-tags text-green-500 mr-2"></i>Tags
                </h2>
                <div class="flex flex-wrap gap-2">
                    ${Array.isArray(data.tags) && data.tags.length > 0
                        ? data.tags.map(tag => `<span class="tag bg-gray-200 text-gray-700 px-3 py-1 rounded-full text-sm cursor-pointer hover:bg-gray-300"><i class="fa-solid fa-tag mr-1"></i>${tag}</span>`).join("")
        ...
        if (data.results && Object.keys(data.results).length > 0) {
            resultsDiv.innerHTML += `<h2 class="text-2xl font-bold text-gray-800 mt-6">
            ...
            sources.forEach(source => {
                if (data.results[source.key] && data.results[source.key].length > 0) {
                    ...
                    resultsDiv.innerHTML += `
                        ...
                        <div id="${contentId}" class="source-content">
                    `;
                    ...
                    data.results[source.key].forEach((item, index) => {
                        ...
                        document.getElementById(contentId).innerHTML += `
                            <div class="result-item bg-white p-4 rounded-lg shadow relative mt-2">
                                <h3 class="text-lg font-semibold flex justify-between items-center">
                                    <span>${index + 1}. ${item.title || "No title available"}</span>
                                    ${sentimentBadge}
                                </h3>
                                <p class="text-gray-700">${item.snippet || "No description available"}</p>
                                ${item.url ? `<a href="${item.url}" target="_blank" class="text-blue-500 hover:underline mt-2 inline-block"><i class="fa-solid fa-external-link mr-1"></i>View Source</a>` : ""}
                            </div>
                        `;
                    });
